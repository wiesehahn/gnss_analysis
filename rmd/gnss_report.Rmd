...
# read log file with nmea data (created with 'GPS Logger for Android')
# dat.name <- "F:/Lehre/gnss/gnss_nmea/data/uebung/GNSS_Daten/s09_samsunggalaxys5_leonhard_meis.nmea"
# dat <- read.table(dat.name, header = FALSE, sep = ",", fill = TRUE, col.names = paste0("V",seq_len(26)))
#NMEA_types <- unique(dat$V1)
#print(NMEA_types)
```
...
```{r table}
dt <- m %>% select(-lon_direction,-lat_direction,-distance) %>% filter(quality != 0)
...
```{r anim.dist, include=FALSE}
# p <- ggplot(m, aes(time, distance)) + 
#   geom_line( size=0.6)+
#   theme_classic() 
# 
# p + 
